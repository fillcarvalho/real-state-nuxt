<template>
  ...
  >
    ...
    <div class="relative leading-1">
      <nuxt-link :to="{name: 'apartamento-slug', params: {slug: property.slug}}" class="overflow-hidden">
        <img
          :src=property.defaultPicture
      ...
      >
        ...
        >
          <nuxt-link :to="{name: 'apartamento-slug', params: {slug: property.slug}}" class="overflow-hidden">
            <i class="flaticon-pin"></i> {{ property.details.location.city }}
          </nuxt-link>
          <ul class="flex gap-10px">
            <li>
              <nuxt-link :to="{name: 'apartamento-slug', params: {slug: property.slug}}" class="w-30px lg:w-42px h-30px flex justify-center items-center bg-transparent lg:bg-white lg:bg-opacity-35 hover:text-secondary-color">
                <font-awesome-icon icon="fa-camera" class="pr-1 leading-1"/>
                {{ property.pictures.length }}
              </nuxt-link>
            </li>
            <li v-if="property.details.video">
              <nuxt-link :to="{name: 'apartamento-slug', params: {slug: property.slug}}" class="w-30px lg:w-42px h-30px flex justify-center items-center bg-transparent lg:bg-white lg:bg-opacity-35 hover:text-secondary-color">
    ...
    <div class="px-5 pt-4 lg:px-30px lg:pt-5">
      ...
      >
        <span class="leading-1.8">R$ {{ property.details.price }}</span>
      </h5>
      <h4
        class="text-lg md:text-xl lg:text-22px font-semibold text-heading-color mb-15px"
      >
        <nuxt-link :to="{name: 'apartamento-slug', params: {slug: property.slug}}" class="hover:text-secondary-color leading-1.3">
          {{ property.name }}
        </nuxt-link>
      </h4>
      <p class="text-sm mb-15px">
                          <span class="leading-1.8"
                          >{{ property.shortDescription }}</span>
      </p>
      ...
      <ul class="flex flex-wrap gap-15px py-4">
        ...
        >
          <p class="leading-1.8 font-bold">
            {{ property.details.bedrooms }}<i class="flaticon-bed ml-1"></i>
          </p>
        ...
        >
          <p class="leading-1.8 font-bold">
            {{ property.details.toilets }}<i class="flaticon-clean ml-1"></i>
          </p>
        ...
        <li class="text-sm">
          <p class="leading-1.8 font-bold">
            {{ property.details.squareMeters }}<i
    ...
    >
      ...
      >
        <div
          ...
          <a :href="`/apartamento/${property.slug}`" class="relative w-full text-center z-10 px-5 md:px-25px lg:px-10 py-10px md:py-3 lg:py-17px group-hover:text-heading-color leading-23px">Veja mais informações <font-awesome-icon icon="fa-arrow-right"/></a>
        </div>
      </div>
    </div>
  </div>
